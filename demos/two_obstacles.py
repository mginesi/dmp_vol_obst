...
obst_volume_2 = obstacle_superquadric.Obstacle_Static(n_dim = 2, n = 1, center = center_2, axis = axis_2)
...
for n in range(num_obst_2):
    obst = point_obstacle.Obstacle_Steering(x = np.array([x_c_2 + a_2*np.cos(t_2[n]), y_c_2 + b_2*np.sin(t_2[n])]), dx_obst = np.zeros(2))
...
def f_point(x, v):
    ...
    for n in range(num_obst_2):
        F += obst_list_2[n].gen_external_force(x, v, dmp.x_goal)
    return F
