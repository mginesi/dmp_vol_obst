...
plt.xlim(-0.75, 0.75)
if savefig:
    plt.savefig('imgs/youbot_static_nullw_always.eps')
...
if savefig:
    plt.savefig('imgs/youbot_static_nullw_always_sol.eps')
...
plt.xlim(-0.75, 0.75)
if savefig:
    plt.savefig('imgs/youbot_dynamic_nullw_always.eps')
...
if saveframes:
    for n in range(int(x_track_dynamic.shape[0] / 10) - 1):
        ...
        plt.axis('scaled')
        plt.xlim(-0.75, 0.75)
        plt.savefig('frames/youbot_dynamic_nullw_always' + str(1000+n) + '.png')
        plt.close()
...
plt.xlabel(r'$t[\texttt{s}]$', fontsize=fs)
if savefig:
    plt.savefig('imgs/youbot_dynamic_nullw_always_sol.eps')
...
plt.xlim(-0.75, 0.75)
if savefig:
    plt.savefig('imgs/youbot_static_nullw_temporal.eps')
...
plt.xlabel(r'$t[\texttt{s}]$', fontsize=fs)
if savefig:
    plt.savefig('imgs/youbot_static_nullw_temporal_sol.eps')
...
plt.xlim(-0.75, 0.75)
if savefig:
    plt.savefig('imgs/youbot_dynamic_nullw_temporal.eps')
...
if saveframes:
    for n in range(int(x_track_dynamic.shape[0] / 10) - 1):
        ...
        plt.axis('scaled')
        plt.xlim(-0.75, 0.75)
        plt.savefig('frames/youbot_dynamic_nullw_temporal' + str(1000+n) + '.png')
        plt.close()
...
plt.axvline(x=t_presence[1], color='k', linestyle='--')
if savefig:
    plt.savefig('imgs/youbot_dynamic_nullw_temporal_sol.eps')
...
if savetraj:
    ...
    traj_interp_null_w_static_always = interp1d(t_static, x_track_static.transpose())
    traj_null_w_static_always = np.transpose(traj_interp_null_w_static_always(np.linspace(t_static[0], t_static[-1], 100)))
    np.save('null_w_static_always', traj_null_w_static_always)
    traj_interp_null_w_dynamic_always = interp1d(t_dynamic, x_track_dynamic.transpose())
    traj_null_w_dynamic_always = np.transpose(traj_interp_null_w_dynamic_always(np.linspace(t_dynamic[0], t_dynamic[-1], 100)))
    np.save('null_w_dynamic_always', traj_null_w_dynamic_always)
    traj_interp_null_w_static_range = interp1d(t_static_temporal, x_track_static_temporal.transpose())
    traj_null_w_static_range = np.transpose(traj_interp_null_w_static_range(np.linspace(t_static_temporal[0], t_static_temporal[-1], 100)))
    np.save('null_w_static_range', traj_null_w_static_range)
    traj_interp_null_w_dynamic_range = interp1d(t_dynamic_temporal, x_track_dynamic_temporal.transpose())
    traj_null_w_dynamic_range = np.transpose(traj_interp_null_w_dynamic_range(np.linspace(t_dynamic_temporal[0], t_dynamic_temporal[-1], 100)))
    np.save('null_w_dynamic_range', traj_null_w_dynamic_range)