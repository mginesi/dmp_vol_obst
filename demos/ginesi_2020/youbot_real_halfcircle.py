...
# MP
MP = dmp_c(n_dmps=2, x_0=x0, x_goal=xg, K=K, alpha_s=alpha, tol=0.02)
t_des = np.linspace(0, 1, 200)
x_des = np.array([ 0.0 + np.cos(1.5*np.pi - t_des * np.pi),
                   1.0 + np.sin(1.5*np.pi - t_des * np.pi)]).transpose()
...
plt.xlim(-1.3, 0.2)
if savefig:
    plt.savefig('imgs/youbot_static_halfcircle_always.eps')
...
if savefig:
    plt.savefig('imgs/youbot_static_halfcircle_always_sol.eps')
...
plt.xlim(-1.3, 0.2)
if savefig:
    plt.savefig('imgs/youbot_dynamic_halfcircle_always.eps')
...
if saveframe:
    for n in range(int(x_track_dynamic.shape[0] / 10) - 1):
        ...
        plt.axis('scaled')
        plt.xlim(-1.3, 0.2)
        plt.savefig('frames/youbot_dynamic_halfcircle_always' + str(n+1000) + '.png')
        plt.close()
...
plt.xlabel(r'$t[\texttt{s}]$', fontsize=fs)
if savefig:
    plt.savefig('imgs/youbot_dynamic_halfcircle_always_sol.eps')
...
plt.xlim(-1.3, 0.2)
if savefig:
    plt.savefig('imgs/youbot_static_halfcircle_temporal.eps')
...
plt.xlabel(r'$t[\texttt{s}]$', fontsize=fs)
if savefig:
    plt.savefig('imgs/youbot_static_halfcircle_temporal_sol.eps')
...
plt.xlim(-1.3, 0.2)
if savefig:
    plt.savefig('imgs/youbot_dynamic_halfcircle_temporal.eps')
...
if saveframe:
    for n in range(int(x_track_dynamic_temporal.shape[0] / 10) - 1):
        ...
        plt.axis('scaled')
        plt.xlim(-1.3, 0.2)
        plt.savefig('frames/youbot_dynamic_halfcircle_temporal' + str(1000+n) + '.png')
        plt.close()
...
plt.axvline(x=t_presence[1], color='k', linestyle='--')
if savefig:
    plt.savefig('imgs/youbot_dynamic_halfcircle_temporal_sol.eps')
...
if savetraj:
    ...
    traj_interp_half_circle_static_always = interp1d(t_static, x_track_static.transpose())
    traj_half_circle_static_always = np.transpose(traj_interp_half_circle_static_always(np.linspace(t_static[0], t_static[-1], 100)))
    np.save('half_circle_static_always', traj_half_circle_static_always)
    traj_interp_half_circle_dynamic_always = interp1d(t_dynamic, x_track_dynamic.transpose())
    traj_half_circle_dynamic_always = np.transpose(traj_interp_half_circle_dynamic_always(np.linspace(t_dynamic[0], t_dynamic[-1], 100)))
    np.save('half_circle_dynamic_always', traj_half_circle_dynamic_always)
    traj_interp_half_circle_static_range = interp1d(t_static_temporal, x_track_static_temporal.transpose())
    traj_half_circle_static_range = np.transpose(traj_interp_half_circle_static_range(np.linspace(t_static_temporal[0], t_static_temporal[-1], 100)))
    np.save('half_circle_static_range', traj_half_circle_static_range)
    traj_interp_half_circle_dynamic_range = interp1d(t_dynamic_temporal, x_track_dynamic_temporal.transpose())
    traj_half_circle_dynamic_range = np.transpose(traj_interp_half_circle_dynamic_range(np.linspace(t_dynamic_temporal[0], t_dynamic_temporal[-1], 100)))
    np.save('half_circle_dynamic_range', traj_half_circle_dynamic_range)