...
# Obstacles
obst_center_1 = np.array([-0.5, 0.7])
obst_axis_1 = np.array([0.3, 0.2])
density_mesh_1 = 50
x_obst_1 = (obst_axis_1[0] * np.cos(np.linspace(0.0, 2*np.pi, density_mesh_1 + 1)) + obst_center_1[0])[:-1]
y_obst_1 = (obst_axis_1[1] * np.sin(np.linspace(0.0, 2*np.pi, density_mesh_1 + 1)) + obst_center_1[1])[:-1]

obst_center_2 = np.array([0.15, 0.4])
obst_axis_2 = np.array([0.1, 0.1])
density_mesh_2 = 50
x_obst_2 = (obst_axis_2[0] * np.cos(np.linspace(0.0, 2*np.pi, density_mesh_2 + 1)) + obst_center_2[0])[:-1]
y_obst_2 = (obst_axis_2[1] * np.sin(np.linspace(0.0, 2*np.pi, density_mesh_2 + 1)) + obst_center_2[1])[:-1]
...
for _i in range(density_mesh_1):
    list_obst_static_point.append(pt_stat(
        x=np.array([x_obst_1[_i], y_obst_1[_i]]), p0=p0, eta=eta))
for _i in range(density_mesh_2):
    list_obst_static_point.append(pt_stat(
        x=np.array([x_obst_2[_i], y_obst_2[_i]]), p0=p0, eta=eta))
...
for _i in range(density_mesh_1):
    list_obst_dynamic_point.append(pt_dyn(
        x=np.array([x_obst_1[_i], y_obst_1[_i]]), lmbda=lmbda, beta=beta))
for _i in range(density_mesh_2):
    list_obst_dynamic_point.append(pt_dyn(
        x=np.array([x_obst_2[_i], y_obst_2[_i]]), lmbda=lmbda, beta=beta))
...
for _i in range(density_mesh_1):
    list_obst_steering_point.append(pt_steer(
        x=np.array([x_obst_1[_i], y_obst_1[_i]]), gamma=gma, beta=beta))
for _i in range(density_mesh_2):
    list_obst_steering_point.append(pt_steer(
        x=np.array([x_obst_2[_i], y_obst_2[_i]]), gamma=gma, beta=beta))
...
A = 10.0
eta = 1.0
obst_stat_vol_1 = sq_stat(center=obst_center_1, axis=obst_axis_1, A=A, eta=eta)
obst_stat_vol_2 = sq_stat(center=obst_center_2, axis=obst_axis_2, A=A, eta=eta)

def ext_f_vs(x, v):
    return obst_stat_vol_1.gen_external_force(x) + obst_stat_vol_2.gen_external_force(x)
...
beta = 2.0
eta = 0.5
obst_stat_vol_1 = sq_dyn(center=obst_center_1, axis=obst_axis_1, lmbda=lmbda, beta=beta, eta=eta)
obst_stat_vol_2 = sq_dyn(center=obst_center_2, axis=obst_axis_2, lmbda=lmbda, beta=beta, eta=eta)

def ext_f_vd(x, v):
    return obst_stat_vol_1.gen_external_force(x, v) + obst_stat_vol_2.gen_external_force(x, v)
...
plt.plot(x_obst_1, y_obst_1, '.k', label='obstacle')
plt.plot(x_obst_2, y_obst_2, '.k', label='obstacle')
...
plt.figure()
plt.plot(gamma[:, 0], gamma[:, 1], '--',color='orange', label='desired')
plt.plot(np.append(x_obst_1,x_obst_1[0]), np.append(y_obst_1, y_obst_1[0]), '-k', label='obstacle')
plt.plot(np.append(x_obst_2,x_obst_2[0]), np.append(y_obst_2, y_obst_2[0]), '-k', label='obstacle')
...
t_range = np.logical_and(t_sp < 0.9, t_sp > 0.05)
